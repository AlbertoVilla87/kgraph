...
# Merging in `GLiNERGraph` (shared core)
...
Merging keeps the node with the best score, accumulates mentions, and updates the entity-type index accordingly. `find_entity` uses the same `canonical` key so relations still resolve against merged nodes. In `SegmentedGLiNERGraph`, a **canonicalization pass** also renames entities via `canonical_map` (e.g. "LLMs" → "Large Language Model") before merging.
...
| `threshold` | — | Reserved for the embedding-similarity merge pass (not yet wired up) |
| `model` | — | Reserved for the embedding-similarity merge pass (not yet wired up) |
...
# Legacy — topic-guided assembly (`DiscoveryAssembly`)

> **Legacy path.** The topic-driven counterpart, **removed** in the model cleanup
> (module and entry points deleted). It failed at seed extraction once KeyBERT had
> no embedding model — see [Discovery → Follow up](discovery.md#follow-up).
...
`uv run assembly-demo` extracted 4 entities and 8 raw relations from `data/case_2/medium.txt`, merged into a final graph of **4 entities and 2 unique relations** (the historical demo output):
...
- [Discovery](discovery.md) — how the taxonomy is produced (citation-guided, plus the legacy topic-guided path)